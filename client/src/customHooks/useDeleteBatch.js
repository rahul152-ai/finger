...
const useDeleteBatch = () => {
  ...
  const deleteBatch = async (batchId) => {
    ...
    const confirmation = await Swal.fire({
      icon: "warning",
      title: "Are you sure?",
      text: "Are you sure want to Delete this Batch and it's Students?",
    ...
    if (!confirmation.isConfirmed) {
      Swal.fire({
        icon: "info",
        title: "Cancelled",
        text: "The batch was not deleted.",
        confirmButtonText: "OK",
      });
    ...
    try {
      ...
      const response = await TableApi.deleteBatch(token, batchId); // API call
      ...
      if (response.status === 200) {
        ...
        Swal.fire({
          icon: "success",
          title: "Batch Deleted",
          text: "The batch has been successfully deleted.",
          confirmButtonText: "OK",
        });
      ...
      let errorMessage =
        "There was an error deleting the batch. Please try again.";
      if (
        error.response &&
        error.response.data &&
